...
@measure_time
def monte_carlo_algorithm(mc_pars, process_pars, ModelPars, det_results): 
    """
    ...
    #* Unpack parameters
    M = det_results[0]     # Monomer concentration, mol/m3
    C = det_results[1]     # Catalyst concentration, mol/m3
    A = det_results[2]     # Acid concentration, mol/m3
    la0 = det_results[3]   # 0th moment of active chains, mol/m3
    la1 = det_results[4]   # 1st moment of active chains, mol/m3
    la2 = det_results[5]   # 2nd moment of active chains, mol/m3
    mu0 = det_results[6]   # 0th moment of dormant chains, mol/m3
    mu1 = det_results[7]   # 1st moment of dormant chains, mol/m3
    mu2 = det_results[8]   # 2nd moment of dormant chains, mol/m3
    ga0 = det_results[9]   # 0th moment of terminated chains, mol/m3
    ga1 = det_results[10]  # 1st moment of terminated chains, mol/m3
    ga2 = det_results[11]  # 2nd moment of terminated chains, mol/m3
    ...
    while time_sim <= time_end:        
        ...
        sumR = np.sum(R)  # Total length of all monomer units in active chains
        sumD = np.sum(D)  # Total length of all monomer units in dormant chains
        sumG = np.sum(G)  # Total length of all monomer units in terminated chains
        
        #* Evaluate individual reaction rates 
        #* rates from ODE       
        # current_t_idx = np.searchsorted(t, time_sim) # For each time_sim look for the closest value in t
        ...
        stepCounter += 1
        if sumRate <1:
            input(f"Pausing since sumRate is very low: {sumRate:.2e}, stepcounter:{stepCounter} press Enter to continue...")
            sumRate = 50
        ...
        # print(f"Time in simulation: {time_sim:.2f}, sumaRate={sumRate:.2e}, tau={tau:.2e}, reaction index={Reac_idx}")
        ...
        if time_sim > out_idx * fq:     # register the data every fq seconds
            ...
            # R_and_D_check = D + R
            # if not np.array_equal(R_and_D, R_and_D_check):
            #     raise RuntimeError("R_and_D and R_and_D_check are not equal; aborting simulation")
    ...
    return t_out, Rates_out, R_out, D_out, G_out, Mn_out, Mw_out, suma_n_tot, RD_column_sums, R, D, G, out_idx
